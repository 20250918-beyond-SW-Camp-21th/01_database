...
-- 2.  서브쿼리는 `WHERE` 절의 연산자 오른쪽에 주로 위치하지만, **`SELECT` 절이나 `FROM` 절에서도 테이블이나 컬럼처럼 사용**될 수 있습니다.
-- 3.  서브쿼리 내에서 `ORDER BY`는 일반적으로 사용할 수 없지만, **`FROM` 절에 사용되는 인라인뷰(Inline View)나 `LIMIT`처럼 특정 순서의 데이터가 꼭 필요한 경우에는 예외적으로 사용**됩니다.
...
-- 열무김치라떼의 카테고리명은 뭘까?
-- 1. tbl_menu에서 열무김치라떼의 카테고리 번호 조회 
SELECT
	   category_code 
  FROM tbl_menu
 WHERE menu_name = '열무김치라떼'; -- result: 8
-- 2. tbl_category에서 해당 카테고리 번호의 카테고리명을 조회 
SELECT
	   category_name 
  FROM tbl_category
...
-- 민트미역국과 같은 카테고리의 메뉴를 조회 (메뉴 코드, 메뉴명, 가격, 주문가능여부 출력)
SELECT
	   menu_code
	 , menu_name
	 , menu_price
...
-- 다중 행 서브쿼리 
-- 서브쿼리의 실행 결과가 여러개의 행으로 나오는 경우 
-- 다중행 연산자는 IN, NOT IN, ANY, ALL 와 함께 사용
-- 서브쿼리가 다중행을 반환하기 때문에 = 연산자를 사용할 수 없음 

-- 모든 식사류 메뉴를 조회하라 
-- '식사류'에 해당하는 카테고리 코드를 서브쿼리로 먼저 조회한 후 
-- 해당 코드들에 포함되는 메뉴들을 찾기
SELECT
	   menu_name
  FROM tbl_menu
...
-- 가장 비싼 메뉴 조회 
-- 서브쿼리가 반환하는 모든 값에 대해 비교 조건이 참이어야 최종적으로 참을 반환
-- MAX 함수로 최대값만 조회 
SELECT 
	   MAX(menu_price)
  FROM tbl_menu
...
-- exists: 조회 결과가 있을 때는 참, 없을 때는 거짓을 반환
...
SELECT
	 ...
	 , orderable_status
  FROM tbl_menu a
 WHERE menu_price = (SELECT MAX(b.menu_price)
 					   FROM tbl_menu b
...
-- 스칼라 서브쿼리 
-- 결과값이 단일값(1개의 행, 1개의 열)인 상관 서브쿼리의 한 종류 
...
SELECT
  ...
  FROM (SELECT COUNT(*) COUNT
		  FROM tbl_menu
		 GROUP BY  category_code) AS countmeun;
