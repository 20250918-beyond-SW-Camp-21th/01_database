...
-- 기본적으로 commit이 자동으로 수행되도록 설정되어 있으므로 변경한 뒤 테스트 진행 
...
DELETE FROM tbl_menu WHERE menu_code = 35;
ROLLBACK;
COMMIT;
...
SELECT * FROM tbl_menu ORDER BY menu_code DESC;
INSERT INTO tbl_menu VALUES(NULL, '바나나해장국', 8500, 4, 'Y');
UPDATE tbl_menu SET menu_name = '수정된 이름' WHERE menu_code = 37;
ROLLBACK;
...
CREATE TABLE kb_bank (
	 ...
	 balance BIGINT CHECK(balance >= 0)    -- 금액이기 때문에 제약조건을 넣음 
);
...
DELETE FROM tbl_menu WHERE menu_code IN (47,48,49);
