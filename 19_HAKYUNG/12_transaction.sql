...
-- 기본적으로 commit이 자동응로 수행되도록 설정되어 있으므로 변경한 뒤 테스트를 한다.
...
INSERT INTO tbl_menu values(NULL, '바나나해장국', 8500, 4, 'Y');
UPDATE tbl_menu SET menu_name = '수정된 메뉴' WHERE menu_code = 31;
ROLLBACK;
SELECT * FROM tbl_menu;
...
CREATE TABLE kb_bank (      -- auto increment 없음
                         id INT PRIMARY KEY ,
...
COMMIT;
-- INSERT INTO tbl_menu(menu_code, menu_name, menu_price, category_code, orderable_status)
-- VALUES (7, '민트미역국', 15000, 4, 'Y');
-- UPDATE tbl_menu SET menu_name = '앙버터김치찜' WHERE menu_code = 5;
-- DELETE FROM tbl_menu WHERE menu_code IN ();
