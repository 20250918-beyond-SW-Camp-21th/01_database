...
--    FROM 절에 사용되는 인라인뷰(Inline View)나 LIMIT처럼 
--     특정 순서의 데이터가 꼭 필요한 경우에는 예외적으로 사용**됩니다.

-- ===================
-- 일반 서브쿼리
-- ===================
-- 단일 행 서브쿼리
-- 서브쿼리의 실행 겨로가가 오직 하나의 행만 나오는 경우를 말하며,
-- 단일행 연산자는 (=, <, <=, >, >=, !=)와 함께 사용된다.
-- 
-- 열무김치라떼의 카테고리 명은 뭘까?
-- 먼저 열무김치라떼의 category_code를 알아내고(서브쿼리),
-- 그 코드를 이용해서 카테고리 명을 찾는다.(메인쿼리)
-- 1. tbl_menu에서 열무김치라떼의 카테고리 번호를 조회
SELECT  
        category_code
    FROM tbl_menu
  WHERE menu_name = '열무김치라떼';
-- 2. tbl_category에서 해당 카테고리 번호의 카테고리 명을 조회(메인쿼리)
SELECT 
        category_name 
    FROM tbl_category 
...
-- 서브쿼리만 실행해보면 결과가 한 행으로 나옴 => 단일행 서브쿼리
...
SELECT
   ...
   WHERE category_code IN (5, 6)
     AND menu_price > ANY(SELECT menu_price
                          FROM tbl_menu
                          WHERE category_code = 4
                         );

-- exists : 존재 여부 확인, 조회 결과가 있을 때 true, 없으면 false
SELECT  
        a.category_name, category_code
    FROM tbl_category a
   WHERE EXISTS (SELECT 1
                   FROM tbl_menu b
                  WHERE b.category_code = a.category_code 
                ); -- tbl_menu에 있는 카테고리 메뉴를 보여준다.
...
SELECT --  인라인뷰에 존재하는 컬럼만 참조할 수 있다.
      ...
      , v.category_name
      , v.menu_price 
    FROM (SELECT menu_name            -- 가상 테이블 v생성
