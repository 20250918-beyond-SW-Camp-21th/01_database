...
-- 단일행 서브쿼리
-- 열무김치라떼의 카테고리명은 뭘까?
-- 먼저 열무김치라떼의 category_code를 알아내고(서브쿼리)
-- 그 코드를 이용해서 카테고리명을 찾는다.
...
-- 다중행 연산자는 (IN, NOT IN, ANY, ALL)과 함께 사용해야 한다
...
-- 서브쿼리가 다중행을 반환하는 경우는 '=' 연산자를 사용할 수 없다.
SELECT
	   *
  FROM
  	   tbl_menu
...
-- 가장 비싼 메뉴 조회
-- 서브쿼리가 반환하는 모드값에 대해 비교 조건이 참이어야 최종적으로 참을 반환
...
-- 인라인뷰(from)로 사용되는 서브쿼리를 미리 정의해서 사용하는 문법
