...
-- 실질적인 데이터를 물리적으로 저장하는 것이 아니라 논리적으로 저장한다.
-- (1) 보안성 
-- (2) 복잡한 구문을 간결하게 작성 가능하다.
...
VALUES (200, '감자탕', 8000, 3, 'Y'); -- view는 4번인데 3번으로 넣으려고 하니 실패

INSERT
  INTO hansik
VALUES (201, '곰탕', 8000, 3, 'Y'); -- 삽입 성공(체크옵션 없음)

INSERT
  INTO hansik_with_check
VALUES (200, '곰탕', 8000, 4, 'Y'); -- 삽입 성공
...
UPDATE hansik_with_check
   SET category_code = 3
 WHERE menu_code = 200; -- 수정 불가. check option 위배
 
 UPDATE hansik
   SET category_code = 3
 WHERE menu_code = 200; -- 수정 가능. check option 없기 때문
