...
-- 기본적으로 commit이 자동으로 수행되도록 설정 돼있으므로 변경한 뒤 테스트를 한다.
...
SELECT * FROM tbl_menu ORDER BY menu_code DESC;
INSERT INTO tbl_menu values(NULL, '바나나해장국', 8500, 4, 'Y');
UPDATE tbl_menu SET menu_name = '수정된이름' WHERE menu_code = 31;
ROLLBACK;
COMMIT;
...
START TRANSACTION;
SELECT menu_code, menu_name, menu_price FROM tbl_menu ORDER BY menu_code DESC;
...
UPDATE tbl_menu SET menu_name = '수정된이름' WHERE menu_code = 5;
DELETE FROM tbl_menu WHERE menu_code IN(40,39,38)

ROLLBACK TO strawberry;
ROLLBACK;

DELETE FROM tbl_menu WHERE menu_code = 31;
COMMIT;