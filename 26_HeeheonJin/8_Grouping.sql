...
-- group by: 결과 집합을 특정 열릐 값에 따라 그룹화
-- having : group by절과 함께 사용하며 그룹의 조건을 적용

-- 그룹함수 : count, sum, avg, min, max (1개)
-- 여러개의 값중에 한개를 반환
-- 카테고별 메뉴의 수를 구하고자한다.
...
-- count 함수의 특성
SELECT 
	   count(*)             -- '*'모든행 count 로하면 전체 행의 갯수
	 , count(category_code) -- 칼람명 기재 값이 있는 행만 카운트
	 , count(ref_category_code)  -- NULL 값은 카운트하지 않는다. Null 빼고 가져왔다. 그래서 category랑 값이 다른거다.
	 FROM tbl_category;
...
-- 카테고리별 메뉴 가격 평균이 10000원 이상인 카테고리의
...
HAVING avg(a.menu_price) >= 10000; -- 조건절 having

-- rollup : 중간 집계 함수로 group by 와 함께 사용
-- group by 절의 척번째 기준 컬럼에 대한 중간 집계 + 총 집계 값이
-- result set에 포함된다. 
