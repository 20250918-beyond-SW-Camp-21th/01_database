...
-- 열무김치라떼의 카테고리명을 뭘까? 
-- 먼저 열무김치라떼의 category_code를 알아내고(서브쿼리), 그 코드를 이용해서 카테고리명을 찾는다. (메인쿼리)
...
-- 민트미역국과 같은 카테고리의 메뉴 조회(메뉴코드, 메뉴명, 가격, 주문가능여부)
-- 아래 코드가 단일행 서브쿼리 예시다. 왜냐하면 서브쿼리 실행하면 단일한 결과가 나오기 때문이다. 
SELECT
       menu_code
     , menu_name
     , menu_price
...
-- 모든 식사류 메뉴를 조회해주세요.
-- '식사류'에 해당하는 카테고리코드를 서브쿼리로 먼저 조회한 후,
-- 해당 코드에 포함(IN)되는 메뉴들을 찾기 
SELECT 
       category_code 
  FROM tbl_category  
...
-- 서브쿼리가 다중행을 반환하는 경우 = 얀신질,ㄹ 시ㅣ영힐 스 앖다. 
-- 예를 들어, 다중행인데 한개만 나오는 코드에서는 연사자가 적용이 당연히 안 된다. 

-- # all 연산자
-- 서브 쿼리의 결과 모두에 대해 연산결과가 참이면 참을 반환한다.
-- x > ALL(..)  : 모든 값보다 크면 참. 최대값보다 크면 참
-- x >= ALL(..) : 모든 값보다 크거나 같으면 참... 
...
SELECT 
	   category_name
  FROM tbl_category a
 WHERE EXISTS (SELECT 1 -- 존재하냐 안 하냐 
 				 FROM tbl_menu b
...
-- (내 이해)가상으로 만들어서 사용하는 것이며 일시적으로 사용하기 위해서 쓴다.
