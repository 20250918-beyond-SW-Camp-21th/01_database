...
-- 기본적으로 commit이 자동으로 수행되도록 설정되 있으므로 변경한 뒤 테스트를 한다.
SET autocommit = 1;   -- autocommit 활성화 됐다. 
SET autocommit = ON;  -- autocommit 활성화 됐다. 
...
DELETE FROM tbl_menu WHERE menu_code = 34;
ROLLBACK;
COMMIT;
...
SELECT * FROM tbl_menu ORDER BY menu_code DESC; 
INSERT INTO tbl_menu VALUES(NULL, '바나나해장국', 8500, 4, 'Y');
UPDATE tbl_menu SET menu_name = '수정된 이름' WHERE menu_code = 27; 
ROLLBACK;
...
SELECT * FROM kb_bank kb; 
SELECT * FROM encore_bank eb;
COMMIT;
START TRANSACTION; -- 깃발하나 
