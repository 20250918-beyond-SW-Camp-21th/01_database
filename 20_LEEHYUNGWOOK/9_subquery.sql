...
-- 9.SUBQUERY
-- ===================================
-- 하나의 SQL문(main-query) 안에 포함되어 있는 또 다른 SQL문(sub-query)으로, 메인 쿼리가 서브 쿼리를 
-- 포함하는 종속적인 관계입니다.
...
-- 3.  서브쿼리 내에서 `ORDER BY`는 일반적으로 사용할 수 없지만, **`FROM` 절에 사용되는 인라인뷰(Inline View)나 
...
-- 열무김치라떼의 카테고리명은 무엇인가?
-- 열무김치라떼의 category_code를 알아내고, 이 코드를 이용해서 카테고리명을 찾는다.
...
-- 2. tbl_category 에서 해당 카테고리 번호의 카테고리명을 조회
SELECT
	   category_name 
  FROM tbl_category
 WHERE category_code = 8;

-- 3. 카테고리 코드 조건 자리에 1번을 삽입한다.
SELECT
	   category_name 
  FROM tbl_category                                        
...
-- 가장 비싼 메뉴 조회
-- 서브쿼리가 반환하는 모든 값에 대해 비교 조건이 참이어야 최종적으로 참을 반환한다.
...
-- ===================================
-- EXISTS : 조회 결과가 있을 땐 TRUE, 없을 때는  FALSE
-- 메뉴 테이블에 존재하는 카테고리만 조회
SELECT category_name
  FROM tbl_category a
 WHERE EXISTS ( SELECT 1 -- <- 존재 여부 1
 			    FROM tbl_menu b
...
-- 해당(각각의?) 카테고리 메뉴의 평균가보다 높은 가격을 가진 메뉴만 조회
SELECT
	   menu_code
	 , menu_name
	 , menu_price
