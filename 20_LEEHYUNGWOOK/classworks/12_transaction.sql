...
START TRANSACTION; -- 트랜잭션 시작 선언

SELECT * FROM tbl_menu ORDER BY menu_code DESC;
INSERT INTO tbl_menu VALUES (null, '바나나해장국', 8500, 4, 'Y');
UPDATE tbl_menu SET menu_name = '수정된이름' WHERE menu_code = 34;
COMMIT;
ROLLBACK;
...
DELETE FROM tbl_menu WHERE menu_name = '민트초코맛파전';

-- 이 때 업데이트가 잘못되어 strawberry 시점으로 돌아가야한다면
