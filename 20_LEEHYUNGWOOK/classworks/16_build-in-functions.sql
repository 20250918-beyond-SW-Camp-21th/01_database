...
-- 문자 탐색시 사용될 수 있는 함수들
SELECT 
       ELT(2, '사과', '딸기', '바나나') -- 해당 위치의 문자열 반환
     , FIELD('딸기', '사과', '딸기', '바나나') -- 찾을 문자열 위치 반환
     , FIND_IN_SET('바나나', '사과,딸기,바나나') -- FIND_IN_SET: 찾을 문자열의 위치 반환
     , INSTR('사과딸기바나나', '딸기') -- INSTR: 기준 문자열에서 부분 문자열의 시작 위치 반환
     , LOCATE('딸기', '사과딸기바나나'); -- LOCATE: INSTR과 동일하고 순서는 반대
...
SELECT ADDDATE('2025-09-18', INTERVAL 30 DAY), ADDDATE('2025-09-18', INTERVAL 6 MONTH);
SELECT SUBDATE('2025-09-18', INTERVAL 30 DAY), SUBDATE('2025-09-18', INTERVAL 6 MONTH);

-- ADDTIME(날짜/시간, 시간), SUBTIME(날짜/시간, 시간)
-- ADDTIME: 날짜 또는 시간을 기준으로 시간을 더함
-- SUBTIME: 날짜 또는 시간을 기준으로 시간을 뺌
SELECT  ADDTIME('2023-05-31 09:00:00', '1:30:00')
	, SUBTIME('2023-05-31 09:00:00', '245:35:1');
...
-- HOUR(시간), MINUTE(시간), SECOND(시간), MICROSEOND(시간)
-- 날짜 또는 시간에서 연, 월, 일, 시, 분, 초, 밀리초를 추출
SELECT YEAR(CURDATE()), MONTH(CURDATE()), DAYOFMONTH(CURDATE());
SELECT HOUR(CURTIME()), MINUTE(CURTIME()), SECOND(CURRENT_TIME), MICROSECOND(CURRENT_TIME);

-- DATE(), TIME(): 일자와 시간 추출 -> NOW() 에서 DATE를 뽑아내고, TIME을 뽑아내는 용도
-- > 원하는 정보만 뽑아서 사용 하는 용도이다. -> currentTIME, DATE랑은 현시점이고 DATE,TIME은 원하는형태 추출이다.
-- DATE: 연-월-일만 추출
-- TIME: 시:분:초만 추출
...
SELECT DATEDIFF('2025-09-18', '2025-09-30'), TIMEDIFF('09:00:00', '10:46:00');
...
SELECT LAST_DAY('20250201');

-- MAKEDATE(연도, 정수)
-- MAKEDATE: 해당 연도의 정수만큼 지난 날짜를 구한다.
SELECT MAKEDATE(2025, 155);
