...
-- ==================================================================
-- 16. build in functions
-- ==================================================================


-- 문제 53: 문자열 합치고 일부만 잘라내기
-- 주제: 문자열 함수 (CONCAT, LEFT)
-- 문제: users 테이블에서 각 사용자의 username과 email을 조합하여, "사용자명 (이메일앞4글자...)" 
-- 와 같은 형태로 출력하세요.
SELECT 	CONCAT(username, ' (', LEFT(email,4), '...@db.com)' )
FROM	users;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT	CONCAT(username, '( ', LEFT(email, 4), '...)') user_profile
FROM 	users;
-- ------------------------------------------------------------------------------------------------------
...
SELECT 	DATE_FORMAT(upload_date, '%X년 %m월 %d일')
FROM	videos;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교: title을 빼먹음
SELECT	title,
		DATE_FORMAT(upload_date, '%Y년 %m월 %d일') upload_date
FROM 	videos
-- ------------------------------------------------------------------------------------------------------
...
SELECT	UPPER(email), LOWER(username)
FROM	users;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
...
SELECT	CHAR_LENGTH(title) title_length, 
...
SELECT	channel_description ,
		REPLACE(channel_description, '니다', '니다!') cd
FROM 	creators;
-- ------------------------------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT 	DISTINCT TRIM(a.category) trimmed_category
FROM 	(
...
SELECT 	LPAD(video_id, 5, 0), video_id
FROM 	videos;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT	video_id, LPAD(video_id, 5, '0') padded_video_id
FROM 	videos;

-- 문제 60: 숫자 반올림 및 올림하기
-- 주제: ROUND(), CEIL()
-- 문제: videos 테이블에서 각 비디오의 재생 시간(duration_seconds)을 60으로 나누어 '분' 단위로 변환하되, 소수점 첫째 자리에서 반올림하여 정수로 표시하고, 소수점을 무조건 올림하여 정수로도 표시해보세요.
SELECT 	CONCAT(ROUND(duration_seconds/60, 1),'분') ROUND, CONCAT(CEIL(duration_seconds/60), '분') CEIL
	, 	CONCAT(duration_seconds, '초') sec
FROM 	videos;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT 	title,
		duration_seconds,
		round(duration_seconds/60, 0) rounded_min, -- > 이 부분이 틀림. 소숫점 첫째 자리에서 반올림 올려야 하는데, 둘째에서 올림
		CEIL(duration_seconds/60) ceiling_min
FROM 	videos;
-- ------------------------------------------------------------------------------------------------------
...
SELECT	CONCAT(ABS(DATEDIFF(upload_date, NOW())), '일') 지남, title
FROM	videos;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT	title,
		upload_date,
		CONCAT(ABS(DATEDIFF(upload_date, NOW())), '일 지남') 지남
FROM	videos;

-- 문제 62: 특정 기간 후의 날짜 구하기
-- 주제: DATE_ADD()
-- 문제: users 테이블에서 각 사용자의 가입일로부터 정확히 100일 후는 며칠인지 '100일 기념일'이라는 이름으로 조회하세요.
SELECT	ADDDATE(registration_date, INTERVAL 100 DAY) '100일 기념일'
FROM	users;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT 	username,
		registration_date,
		DATE_ADD(registration_date, INTERVAL 100 DAY) '100일 기념일'
FROM	users;
...
SELECT 	IFNULL(COALESCE(channel_description), '설명이 없습니다.') -- > IFNULL과 COALESCE 중 하나만 해도 됨
FROM	creators;
SELECT * FROM creators;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT  channel_name,
		COALESCE(channel_description, '설명이 없습니다.') description_check
FROM	creators;
...
SELECT	IF (view_count > 1000000, '인기 영상', '일반 영상') '영상 등급',
		title,
		view_count
FROM 	videos;
-- ------------------------------------------------------------------------------------------------------
...
SELECT 	CONCAT('[',CAST(video_id AS VARCHAR(10)),']	', title)
FROM	videos;
-- ------------------------------------------------------------------------------------------------------
-- 정답비교
SELECT	CONCAT('[', CAST(video_id AS CHAR), '] ', title) formatted_title
FROM 	videos;
-- ------------------------------------------------------------------------------------------------------
...
SELECT	SUBSTRING_INDEX(email, '@', -1) domain_ext, 
		COUNT(username)
FROM 	users
GROUP BY
		domain_ext;
...
JOIN	videos b ON b.video_id = c.video_id;
-- -------------------------------------------------------------------------------------------
CREATE VIEW v_user_likes AS
SELECT 	a.username,
		c.title ,
...
SELECT	username, title, like_datetime
FROM 	v_user_likes vul
WHERE	username = '겜프로'
ORDER BY
		like_datetime DESC;
-- -------------------------------------------------------------------------------------------
...
SELECT  cre.channel_name 채널이름,
		COUNT(vid.video_id) 비디오개수,
		SUM(vid.view_count ) 총조회수,
		CEILING(AVG(vid.view_count )) 평균조회수
FROM	creators cre
JOIN	videos vid
		ON vid.creator_id = cre.creator_id
GROUP BY 
		cre.channel_name; 
-- -------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_creator_stats1 AS
SELECT  a.channel_name 채널이름,
		COUNT(b.video_id) 비디오개수,
		SUM(b.view_count ) 총조회수,
		FLOOR(AVG(b.view_count )) 평균조회수
FROM	creators a
LEFT JOIN	videos b
...
FROM	users;
-- -------------------------------------------------------------------------------------------
CREATE VIEW v_public_users1 AS
SELECT 	user_id ,
		username ,
		country
FROM	users;
...
SELECT 	u.user_id ,
		u.username ,
		u.country ,
		IF (u.user_id = cre.user_id, cre.channel_name , null) 크리에이터면
FROM	users u
-- JOIN	creators cre ON u.user_id = cre.user_id; --> LEFT를 하지 않으면 정보가 짤린다.
LEFT JOIN	creators cre ON u.user_id = cre.user_id;
-- -------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_public_users1 AS
SELECT 	u.user_id ,
		u.username ,
		u.country ,
		cre.channel_name
FROM	users u
LEFT JOIN	
		creators cre ON u.user_id = cre.user_id;
-- -------------------------------------------------------------------------------------------
