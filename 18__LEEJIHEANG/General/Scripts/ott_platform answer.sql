...
-- 문제: 조회수(view_count)가 1,000,000 (백만) 회를 초과하는 
-- 모든 비디오의 creator_id, title, view_count를 조회하세요.
SELECT
       creator_id
     , title
     , view_count
...
-- 문제: users 테이블의 모든 사용자를 가입일(registration_date)이 
--      최신인 순서대로 정렬하여 모든 정보를 조회하세요.
SELECT 
       *
  FROM users
...
-- 문제: 'IT/테크' 카테고리의 비디오 중에서 조회수가 500,000 이상인 
--      비디오의 제목, 카테고리, 조회수를 조회하세요.
SELECT
       title 제목
     , category 카테고리
     , view_count 조회수
...
-- 문제: creators 테이블에서 creator_id가 1, 3, 5, 7인 크리에이터의 채널명과 
-- creator_id를 조회하세요.
SELECT 
       channel_name 
     , creator_id 
...
-- 문제: 'IT/테크' 카테고리이거나 '교육' 카테고리이면서, 
-- 동시에 재생 시간이 1시간(3600초) 이상인 비디오의 
...
-- 문제: 모든 비디오를 카테고리(오름차순) 순으로 먼저 정렬하고, 
-- 같은 카테고리 내에서는 조회수(내림차순)가 높은 순으로 정렬하여 
...
-- 문제: 크리에이터의 채널 설명(channel_description)에 
-- '튜토리얼'이라는 단어가 포함되지 않은 채널의 이름을 조회하세요.
SELECT
       channel_name 
  FROM creators
...
-- 문제: 재생 시간(duration_seconds)이 10분(600초)에서 30분(1800초) 사이가 아닌 
--      비디오의 제목과 재생 시간을 조회하세요.
SELECT 
       title
     , duration_seconds 
  FROM videos
...
-- 문제: '여행' 카테고리 비디오 중 조회수가 500,000 이상인 경우 또는 
-- '음악' 카테고리 비디오 중 재생 시간이 5분(300초) 이하인 경우를 모두 조회하세요. 
--  (결과에는 제목, 카테고리, 조회수, 재생 시간을 포함하세요.)
SELECT 
       title
     , category
     , view_count
...
-- 문제: 모든 사용자를 user_id 순으로 정렬했을 때, 
-- 11번째부터 20번째까지(10건)의 사용자 이름과 이메일을 조회하세요.
SELECT 
       username
     , email
  FROM users
...
-- 문제: 'IT/테크', '게임', '교육' 카테고리를 제외한 나머지 카테고리의 비디오 제목과 
--      카테고리를 조회하세요.
SELECT 
       title
     , category
...
-- 문제: users 테이블에서 사용자 이름(username)이 '겜'으로 시작하고 
--      총 세 글자인 사용자를 조회하세요.
SELECT 
       username
  FROM users
...
-- 문제: 국적이 '대한민국' 또는 '미국'인 사용자 중, 사용자 이름(username)의 
--      가나다순으로 정렬하여 상위 5명만 user_id, username, country를 조회하세요.
SELECT 
       user_id
     , username
     , country
