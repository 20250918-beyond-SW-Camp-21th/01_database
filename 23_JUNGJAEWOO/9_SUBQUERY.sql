...
-- SUBQUERY
-- ===================================
-- 하나의 SQL문(main-query) 안에 포함되어 있는 또 다른 SQL문(sub-query)으로, 
...
-- 2.  서브쿼리는 `WHERE` 절의 연산자 오른쪽에 주로 위치하지만, **`SELECT` 절이나 `FROM` 절에서도 
--     테이블이나 컬럼처럼 사용**될 수 있습니다.
-- 3.  서브쿼리 내에서 `ORDER BY`는 일반적으로 사용할 수 없지만, 
--     **`FROM` 절에 사용되는인라인뷰(Inline View)나 `LIMIT`처럼 특정 순서의 데이터가 
--     꼭 필요한 경우에는 예외적으로 사용**됩니다.
...
-- 열무김치라떼의 카테고리명은 뭘까?
-- 먼저 열무김치라떼의 category_code를 알아내고(서브쿼리), 그 코드를 이용해서 카테고리 명을 찾는다.(메인쿼리)
...
-- x != ANY(..): 서브쿼리의 값 중 어느 하나와 다르면 참입니다. 
--               즉, 서브쿼리 결과에 x와 다른 값이 하나라도 존재하면 참이 됩니다.
...
SELECT 
	   *
  FROM tbl_menu
 WHERE menu_price >= ALL(SELECT MAX(menu_price)
 					 	   FROM tbl_menu
						);
...
SELECT
 ...
 ORDER BY a.category_code; -- 당연히 서브쿼리 절에서 안되는거지 메인은 되네
